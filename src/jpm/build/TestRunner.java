...
public class TestRunner {
    ...
    public TestRunResult runTests(
        ...
        try {
            ...
            generateJUnitReport(reportDir, testResults, totalTests, passedTests, failedTests, skippedTests);
    ...
    private String getClassName(File rootDir, File classFile) {
        var relativePath = classFile.getAbsolutePath()
    ...
    private void generateJUnitReport(
        ...
        try (var writer = new PrintWriter(new FileWriter(reportFile))) {
            ...
            writer.printf("  <testsuite name=\"jpm-tests\" tests=\"%d\" failures=\"%d\" skipped=\"%d\" time=\"0.0\">%n",
            ...
            for (var result : results) {
                ...
                writer.printf("    <testcase classname=\"%s\" name=\"%s\" time=\"%.3f\">%n",
                    className, methodName, result.time() / 1000.0);
                
                if (result.status().equals("FAILED")) {
                    writer.printf("      <failure message=\"%s\"/>%n",
     ...
     */
    private record ClassResult(int total, int passed, int failed, int skipped, List<TestResult> details) {}
